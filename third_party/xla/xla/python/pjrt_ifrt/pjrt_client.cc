...
absl::StatusOr<std::unique_ptr<PjRtClient>> PjRtClient::Create(
  ...
  if (options.transfer_server_factory.has_value()) {
    options.cross_host_dcn_max_num_parallel_copies =
        client->addressable_device_count() * 2;
    TF_ASSIGN_OR_RETURN(client->transfer_server_,
                        (*options.transfer_server_factory)(options));
  }
...
PjRtClient::CopyArraysForCrossHostFallback(
  ...
  {
    absl::MutexLock lock(&(*transfer_server_)->socket_server_mu_);
    TF_RETURN_IF_ERROR(
        (*transfer_server_)
            ->StartTransferServerIfNotStarted(shared_ptr_pjrt_client(),
                                              kv_store_, process_index()));
  }
  ...
  for (auto& [dst_process_index, src_process_indices] : process_index_map) {
    for (auto& src_process_index : src_process_indices) {
      ...
      if (src_process_index == process_index()) {
        ...
        TF_RETURN_IF_ERROR(
            (*transfer_server_)
                ->CrossHostPull(shared_ptr_pjrt_client(), kv_store_, uuid,
                                arrays, pull_to_device_idxs[src_process_index],

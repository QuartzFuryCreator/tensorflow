...
class TransferServerInterface {
 public:
  ...
  // Starts the transfer server if it is not already started.
  virtual absl::Status StartTransferServerIfNotStarted(
      std::shared_ptr<xla::PjRtClient> pjrt_client,
      std::shared_ptr<xla::KeyValueStoreInterface> kv_store, int process_index)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(socket_server_mu_) = 0;

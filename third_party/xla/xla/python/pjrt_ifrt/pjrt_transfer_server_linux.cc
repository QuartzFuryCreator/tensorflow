...
PjRtTransferServerLinux::PjRtTransferServerLinux(
    size_t transfer_size, size_t max_num_parallel_copies,
...
absl::StatusOr<std::unique_ptr<PjRtTransferServerLinux>>
PjRtTransferServerLinux::Create(xla::ifrt::PjRtClient::CreateOptions& options) {
  TF_ASSIGN_OR_RETURN(aux::SocketAddress socket_address,
                      aux::SocketAddress::Parse(options.socket_address));
  std::vector<aux::SocketAddress> transport_addresses;
  if (options.transport_addresses.empty()) {
    TF_ASSIGN_OR_RETURN(aux::SocketAddress socket_address,
                        aux::SocketAddress::Parse("0.0.0.0:0"));
    // TODO(emilyaf, parkers): Remove this once defaults are set per device
    // platform.
    transport_addresses.reserve(4);
    for (int i = 0; i < 4; ++i) {
      transport_addresses.push_back(socket_address);
    }
  } else {
    transport_addresses.reserve(options.transport_addresses.size());
    for (const std::string& transport_address : options.transport_addresses) {
      TF_ASSIGN_OR_RETURN(aux::SocketAddress socket_address,
  ...
  return std::make_unique<PjRtTransferServerLinux>(
      options.cross_host_dcn_transfer_size,
      options.cross_host_dcn_max_num_parallel_copies,
      options.cross_host_transfer_timeout, socket_address, transport_addresses);
}

absl::Status PjRtTransferServerLinux::StartTransferServerIfNotStarted(
  ...
  TF_RETURN_IF_ERROR(
      kv_store->Set(absl::StrCat(kKeyPrefixSocketAddress, process_index),
  ...
  TF_ASSIGN_OR_RETURN(
      auto map, aux::MapPjrtMemory(pjrt_client, tmp->data(), tmp->size(), tmp));
  ...
  TF_ASSIGN_OR_RETURN(
      auto mem, aux::AllocateAndMapPjrtMemory(pjrt_client, total_size * 2));
...
absl::Status PjRtTransferServerLinux::CrossHostAwaitPull(
...
absl::Status PjRtTransferServerLinux::CrossHostPull(
    std::shared_ptr<xla::PjRtClient> pjrt_client,
    ...
    xla::ifrt::DeviceListRef dst_devices, std::optional<MemoryKind> memory_kind,
    int remote_pid, absl::btree_map<int, PjRtArray::PjRtBuffers>& buffer_list) {
  if (!socket_server_.has_value()) {
    return absl::InternalError("Socket server is not initialized.");
  }
  TF_ASSIGN_OR_RETURN(
      std::string address_str,
      kv_store->Get(absl::StrCat(kKeyPrefixSocketAddress, remote_pid),

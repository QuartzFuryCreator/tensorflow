...
TEST_F(ExtractSplitDimSpecTest, SingleDim) {
  absl::string_view hlo_string = R"(
    ...
    ENTRY main {
      ...
      ROOT ds = f32[8,10] dynamic-slice(param, ds_index, zero), dynamic_slice_sizes={8,10}
    }
  )";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnUnverifiedModule(hlo_string));
  auto* ds = module->entry_computation()->root_instruction();
...
TEST_F(ExtractSplitDimSpecTest, MultipleDimOffsets) {
  absl::string_view hlo_string = R"(
    ...
    ENTRY main {
      ...
      ROOT ds = f32[8,10] dynamic-slice(param, zero, ds_index1), dynamic_slice_sizes={8,10}
    }
  )";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnUnverifiedModule(hlo_string));
...
TEST_F(ExtractSplitDimSpecTest, MultipleSplitDimsAllowed) {
  absl::string_view hlo_string = R"(
    ...
    ENTRY main {
      ...
      ROOT ds = f32[16,10,15] dynamic-slice(param, zero, ds_index1, ds_index2), dynamic_slice_sizes={16,10,15}
    }
  )";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnUnverifiedModule(hlo_string));
...
TEST_F(ExtractSplitDimSpecTest, MultipleSplitDimsNonConsecutive) {
  absl::string_view hlo_string = R"(
    ...
    ENTRY main {
      ...
      zero = s32[] constant(0)
      ds_index2 = s32[] parameter(2)
      ROOT ds = f32[8,20,15] dynamic-slice(param, ds_index0, zero, ds_index2), dynamic_slice_sizes={8,20,15}
    }
  )";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnUnverifiedModule(hlo_string));
...
ENTRY main {
  param = f32[16,10] parameter(0)
  zero = s32[] constant(0)
  ROOT ds = f32[16,10] dynamic-slice(param, zero, zero), dynamic_slice_sizes={16,10}
}
)";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnUnverifiedModule(hlo_string));
